...
describe('D5 — Extension Context Invalidation (content.js line 941)', () => {
  ...
  describe('Overlay cleanup on context invalidation', () => {
    it('should hide overlay when context is invalidated', () => {
      ...
      const handler = (event) => {
        ...
        if (msg.includes('Extension context invalidated') || !chrome.runtime?.id) {
          hideOverlay();
          const styleEl = {
            getAttribute: vi.fn().mockReturnValue('ssa-overlay-style'),
          };
          document.getElementById = vi.fn((id) => {
            if (id === 'ssa-overlay-style') return styleEl;
            return null;
          });
          styleRemoved = true;
        }
    ...
    it('should remove overlay style element on invalidation', () => {
      const styleEl = {
        id: 'ssa-overlay-style',
        remove: vi.fn(),
      };
    ...
    it('should handle null style element gracefully', () => {
    ...
    it('should handle overlayEl being null during invalidation', () => {
      let overlayEl = null;
    ...
    it('should handle miniBadgeEl cleanup on invalidation', () => {
      const miniBadgeEl = {
        remove: vi.fn(),
      };
      ...
      expect(miniBadgeEl.remove).toHaveBeenCalled();
    });
  });
  ...
  describe('Cleanup on context invalidation — edge cases', () => {
    it('should handle document.getElementById returning element without remove method', () => {
      ...
      // Element has no remove() — should throw TypeError
      expect(threw).toBe(true);
    });

    it('should handle document.getElementById throwing', () => {
      document.getElementById = vi.fn(() => {
    ...
    it('should handle overlayEl.remove throwing', () => {
      const overlayEl = {
        remove: vi.fn(() => {
    ...
    it('should handle all DOM elements being null during invalidation', () => {
      const overlayEl = null;
      const miniBadgeEl = null;
