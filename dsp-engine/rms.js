...
/**
 * Class for calculating RMS values from audio buffers
 */
class RMS {
  constructor() {
    this.sampleCount = 0;
  ...
  /**
   * Static utility method for one-time dBFS calculation
   * @param {Float32Array|number[]} buffer - Audio sample buffer
   * @returns {number} RMS value in dBFS
   */
  static calculateDBFSStatic(buffer) {
    return new RMS().calculateDBFS(buffer);
  }
